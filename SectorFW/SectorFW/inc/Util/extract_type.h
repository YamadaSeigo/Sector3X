...
namespace SectorFW
{
	 ...
	 */
    template<typename T>
    std::optional<T> extract_first_of_type() {
        return std::nullopt;
    }
	/**
    ...
    std::optional<T> extract_first_of_type(First&& first, Rest&&... rest) {
     ...
     */
    template<typename T, typename First, typename... Rest>
    T extract_required_type(First&& first, Rest&&... rest) {
        if constexpr (std::is_same_v<std::decay_t<First>, T>) {
            return T(std::forward<First>(first));
        }
        else {
