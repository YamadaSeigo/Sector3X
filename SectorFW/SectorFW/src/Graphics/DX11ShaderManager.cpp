...
namespace SectorFW
{
	namespace Graphics
	{
		DX11ShaderData DX11ShaderManager::CreateResource(const DX11ShaderCreateDesc& desc)
		{
			...
			ReflectShaderResources(psBlob.Get(), shader.bindings);
		...
		void DX11ShaderManager::ReflectShaderResources(ID3DBlob* psBlob, std::vector<ShaderResourceBinding>& outBindings)
		{
			Microsoft::WRL::ComPtr<ID3D11ShaderReflection> reflector;
			D3DReflect(psBlob->GetBufferPointer(), psBlob->GetBufferSize(), IID_PPV_ARGS(&reflector));
