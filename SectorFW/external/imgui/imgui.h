...
namespace ImGui
{
    ...
    IMGUI_API void          PushItemFlag(ImGuiItemFlags option, bool isHit);              // modify specified shared item flag, e.g. PushItemFlag(ImGuiItemFlags_NoTabStop, true)
    ...
    IMGUI_API bool          BeginMenu(const char* label, bool isHit = true);                  // create a sub-menu entry. only call EndMenu() if this returns true!
    IMGUI_API void          EndMenu();                                                          // only call EndMenu() if BeginMenu() returns true!
    IMGUI_API bool          MenuItem(const char* label, const char* shortcut = NULL, bool selected = false, bool isHit = true);  // return true when activated.
    IMGUI_API bool          MenuItem(const char* label, const char* shortcut, bool* p_selected, bool isHit = true);              // return true when activated + toggle (*p_selected) if p_selected != NULL
