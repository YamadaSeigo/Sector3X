...
namespace SFW
{
	namespace Graphics::DX11
	{
		...
		struct ModelAssetCreateDesc {
			...
			float meltFillPct = 0.3f;		// ボクセルを埋める割合のしきい値（0.0...1.0）
			float minWorldSizeM = 1.0f;    // 小さすぎるモデルはOccluder対象外（対角長[m]）
			float minThicknessRatio = 0.01f;// 最小厚み比。これ未満は超薄板として減点
			MeltBoxType meltBoxType = MeltBoxType::REGULAR; // meltのボックスタイプ

			enum OccluderBoxType : uint8_t
			{
		...
		struct ModelAssetData {
			...
			uint8_t flags = 0; //描画コマンド用フラグ（Graphics::DrawFlag参照）
		private:
		...
		public:
			...
			//シェーダーのバインド名と合わせる
			//===========================================================================================
			static inline constexpr const char* gMaterialBindName = "MaterialCB";
			static inline constexpr const char* gSamplerBindName = "gSampler";
