...
namespace SectorFW
{
	namespace Physics
	{
		...
		inline JPH::Quat ToJQuat(const Quatf& q) { return JPH::Quat(q.x, q.y, q.z, q.w); }
		inline JVec3     ToJVec3(const Vec3f& v) { return JVec3(v.x, v.y, v.z); }
		...
		inline Vec3f FromJVec3(const JVec3& v) { return Vec3f{ (float)v.GetX(), (float)v.GetY(), (float)v.GetZ() }; }
		inline Quatf FromJQuat(const JPH::Quat& q) { return Quatf{ q.GetX(), q.GetY(), q.GetZ(), q.GetW() }; }
