...
namespace SectorFW {
	namespace Physics {
		...
		public:
			using ShapeResolverFn = std::function<ShapeHandle(ECS::EntityID)>;
			...
			void SetContext(const ServiceLocator& serviceLocator) noexcept {
				ps = serviceLocator.Get<PhysicsService>();
				if (!ps) {
			...
			ECS::AccessInfo GetAccessInfo() const noexcept override {
				...
				return ECS::ComponentAccess<ECS::Read<PhysicsInterpolation>, ECS::Read<BodyComponent>>::GetAccessInfo();
			}

		private:
			ShapeResolverFn resolveShape;
			PhysicsService* ps = nullptr; // PhysicsService へのポインタ（サービスロケーターから取得）
