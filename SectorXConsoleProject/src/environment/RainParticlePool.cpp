...
void RainParticlePool::Create(ID3D11Device* dev)
{
	...
	m_pointSampler = CreateSamplerState(dev, D3D11_FILTER_MIN_MAG_MIP_POINT, D3D11_TEXTURE_ADDRESS_CLAMP, D3D11_TEXTURE_ADDRESS_CLAMP, D3D11_TEXTURE_ADDRESS_CLAMP);
	...
	uint32_t frameSpawnCount)
{
   ...
   // -----------------------------
	...
	{
		// SRV
		ctx->CSSetShaderResources(0, 1, m_aliveCountRaw.srv.GetAddressOf());
		...
		uint32_t totalThreads = (std::min)(frameSpawnCount, MaxSpawnPerFrame); // CBと一致させる
	...
	ID3D11ShaderResourceView* psSrvs[] = {
		lightData->normalLightSRV,
		lightData->fireflyLightSRV,
		lightData->lightCountSRV,
		lightData->lightIndexSRV
	};
	...
	ctx->PSSetConstantBuffers(0, 2, psCBs);
	...
	// VS SRV unbind（次のパスで競合しにくくする）
	{
		...
		ctx->PSSetShaderResources(0, 4, nullVsSrvs);
	}
}