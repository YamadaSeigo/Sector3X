...
namespace App
{
	...
	bool Application::InitializeGraphics()
	{
		...
		auto samplerMgr = m_graphics.GetRenderService()->GetResourceManager<Graphics::DX11::SamplerManager>();
		Graphics::DX11::SamplerCreateDesc sampDesc{};
		sampDesc.name = "LinearWrap";
		sampDesc.desc.Filter = D3D11_FILTER_MIN_MAG_MIP_LINEAR;
		sampDesc.desc.AddressU = D3D11_TEXTURE_ADDRESS_WRAP;
		sampDesc.desc.AddressV = D3D11_TEXTURE_ADDRESS_WRAP;
		sampDesc.desc.AddressW = D3D11_TEXTURE_ADDRESS_WRAP;
		Graphics::SamplerHandle sampHandle = {};
		auto linearSampler = samplerMgr->CreateResource(sampDesc, sampHandle);
		m_linearSampler = linearSampler.state;

		sampDesc.name = "PointClamp";
		sampDesc.desc.Filter = D3D11_FILTER_MIN_MAG_MIP_POINT;
		auto pointSampler = samplerMgr->CreateResource(sampDesc, sampHandle);
		m_pointSampler = pointSampler.state;
